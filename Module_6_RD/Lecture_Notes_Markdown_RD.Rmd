...
In a sharp RD design, the probability that $D=1$ changes from 0 to 1 as the running variable crosses $c$. In other words, no one with $X<c$ gets treated and everyone with $X\geq c$ gets treated, thus, $D_i$ is a deterministic function of $X_i: D_i=1(X_i\geq c)$.
...
$$
0<lim_{x\rightarrow c}P(D-i=1|X_i=x)-lim_{x\leftarrow c}P(D_i=1|X-i=x)<1
$$
...
$$
Y_i=\pi_0+\pi_1Z_i+\pi_2(X_i-c)+\pi_3(X_i-c)*Z_i+u_i 
$$
and 
