...
- The $\beta_j$'s still have a useful interpretation though: each $\beta_j$ measures the change in the probability of sucess when $x_j$ changes by one unit holding all other factors constant.
...
The graph below depicts the probability of labor force participation and $educ$. The other independent variables, for the purpouses of this example, are fixed at the values $nwifeinc=50$, $exper=5$, $age=30$, $kidslt6=1$ and $kidsge6=0$.
...
2) A related problem: a linear probability model implies that the oucome is linearly related to the independent variables. THis is not possible with probabilities. In the previous example, an additional child reduces the probability of working by 0.262. Thus our model implies that going from 0 to 4 young children reduces the probability by 0.262*4=1.048, 104.8 percentage points, which is impossible.
...
where $\Lambda$ is commonly used notation to represent the complext logit function.
...
Recall that by default this computes the average marginal effect for the sample. To insead compute the marginal effect for a particular type of observation
